### AIC simulation study: GB2 income share generation ### 
...
# simulation for GB2 being the true income share generating model

gen <- foreach(d = data_points, .combine = 'list', .multicombine = T) %do% {
  pop <- generate_pop(d)
  inc <- generate_income(pop= pop, model = list(GB2 = gb2))
  list(pop, inc)
  }
...
save(chosen, file = 'gb2_output.RData') # save results
