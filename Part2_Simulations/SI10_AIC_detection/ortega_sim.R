### AIC simulation study: ORTEGA income share generation ### 
...
# simulation for Ortega being the true income share generating model

gen <- foreach(d = data_points, .combine = 'list', .multicombine = T) %do% { 
  pop <- generate_pop(d)
  inc <- generate_income(pop= pop, model = list(ORTEGA = ortega))
  list(pop, inc)
  }
...
system.time(chosen <- foreach (n = 1:length(data_points), .combine = 'rbind', .multicombine = T)%:%
             foreach( i=1:sim, .combine = 'rbind', .packages = "dplyr")%dopar% {
...
#chosen <- data.frame(matrix(nrow = sim*length(data_points), ncol = 2))
save(chosen, file = 'ortega_output.RData')
